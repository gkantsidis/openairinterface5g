              ...
              n_iter_stats_t *dec_iter)
{
  ...
  *crc_misses = 0;
  ...
  printf("ldpc_test: codeword_length %d, n_segments %d, block_length %d, BG %d, Zc %d, Kb %d\n",
         n_segments * block_length, n_segments, block_length, configuration.BG, Zc, configuration.Kb);

  no_punctured_columns = (int)((configuration.nrows - 2) * Zc + block_length - block_length * (1 / ((float)nom_rate / (float)denom_rate))) / Zc;
  //  printf("puncture:%d\n",no_punctured_columns);

  removed_bit = (configuration.nrows - no_punctured_columns - 2) * Zc + block_length - (int)(block_length / ((float)nom_rate / (float)denom_rate));
  if (ntrials == 0)
  {
    ldpc_encoder_orig(test_input[0], channel_input[0], block_length, configuration.BG, 1);
  }
  ...
  for (trial = 0; trial < ntrials; trial++)
  {
    ...
    {
      ldpc_encoder_orig(test_input[j], channel_input[j], block_length, configuration.BG, 0);
    }
    stop_meas(&time);
    ...
    {
      start_meas(time_optim);
      ldpc_encoder_optim_8seg_multi(test_input, channel_input_optim, block_length, configuration.BG, n_segments, j, &tinput, &tprep, &tparity, &toutput);
      stop_meas(time_optim);
    }
    ...
    if (trial == 0)
    {
      ...
      printf("number of undecoded bits: %d\n", (configuration.Kb + configuration.nrows - no_punctured_columns - 2) * Zc - removed_bit);
    }
    ...
    { // Transmitting one segment
      for (j = 0; j < n_segments; j++)
      {
        for (i = 2 * Zc; i < (configuration.Kb + configuration.nrows - no_punctured_columns) * Zc - removed_bit; i++)
        {
#ifdef DEBUG_CODER
          if ((i & 0xf) == 0)
            printf("\ne %d..%d:    ", i, i + 15);
  ...
  *errors_bit_uncoded = *errors_bit_uncoded / (double)((configuration.Kb + configuration.nrows - no_punctured_columns - 2) * Zc - removed_bit);
...
int main(int argc, char *argv[])
{
  ...
  char fname[200];
  sprintf(fname, "ldpctest_BG_%d_Zc_%d_rate_%d-%d_block_length_%d_maxit_%d.txt", configuration.BG, Zc, nom_rate, denom_rate, block_length, No_iteration);
